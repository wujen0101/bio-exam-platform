...
function parseSingleQuestion(block, unitId, chapterId, globalLineOffset) {
  ...
  const noMatch = headerLine.match(/題目\s*[A-Za-z一-鿿]?(\d+)/)
  ...
  for (let i = 1; i < block.length; i++) {
    ...
    // 舊格式：「📝 題目」/「emoji 題目」/「[Q] 題目」單獨一行，需排除「題目 N」標題行
    if (zhStart === -1 && enStart === -1 &&
        !l.includes('中文版') && !l.includes('英文') && !l.match(/題目\s*[A-Za-z一-鿿]?\d/) &&
        (l.trimStart().startsWith('題目') || (l.includes('題目') && (l.includes('📝') || l.trimStart().startsWith('emoji') || /^\[Q\]/.test(l))))) {
      zhStart = i + 1
    }
